...
public class RequirementGenerationService : IRequirementGenerationService
{
    ...
    )
    {
        ...
        if (category != NvosCategory.IV)
        {
            AddReq(
                ...
                "КоАП РФ 8.41/8.41.1",
                "Штраф (КоАП 8.41): 50-100 тыс. руб." // <-- Добавлено ИИ
            );
            ...
            AddReq(
                ...
                "ФЗ-7, приказы РПН",
                "Штраф (КоАП 8.5): 20-80 тыс. руб." // <-- Добавлено ИИ
            );
            ...
            AddReq(
                ...
                "Штраф (КоАП 8.1): 20-100 тыс. руб." // <-- Добавлено ИИ
            );
        }
        ...
        AddReq(
            ...
            "Приказ Минприроды",
            "Штраф (КоАП 8.2): 100-250 тыс. руб." // <-- Добавлено ИИ
        );
        ...
        AddReq(
            ...
            "ФЗ-7",
            "Штраф (КоАП 8.21 ч.1): 100-200 тыс. руб." // <-- Добавлено ИИ
        );
        ...
        AddReq(
            ...
            "СанПиН",
            "Штраф (КоАП 6.3): 10-20 тыс. руб. / приостановка" // <-- Добавлено ИИ
        );
        ...
        AddReq(
            ...
            "Приказ Росстата",
            "Штраф (КоАП 13.19): 20-70 тыс. руб." // <-- Добавлено ИИ
        );
        ...
        {
            AddReq(requirements, "Экспертное заключение и СЭЗ на проект НДВ", "Роспотребнадзор"); // <-- Изменено ИИ
        ...
        switch (waterUse)
        {
            ...
            case WaterUseType.River: // Река/Озеро
                AddReq(
                    ...
                    "Водный кодекс РФ",
                    "Штраф (КоАП 7.6): 50-100 тыс. руб." // <-- Добавлено ИИ
                );
                break;
        }
        ...
        if (hasByproducts)
        {
            AddReq(
                ...
                "ГОСТ/ТУ",
                "Риск (ФЗ-248): 250-350 тыс. руб." // <-- Добавлено ИИ
            );
        }
    ...
    /// (Изменен ИИ: добавлен 'penaltyRisk')
    /// </summary>
    private void AddReq(
        List<EcologicalRequirement> list,
        string title,
        string basis,
        string? penaltyRisk = null // <-- Добавлено ИИ (опционально)
    )
    {
        list.Add(
            new EcologicalRequirement
            {
                Title = title,
                Basis = basis,
                PenaltyRisk = penaltyRisk, // <-- Добавлено ИИ
